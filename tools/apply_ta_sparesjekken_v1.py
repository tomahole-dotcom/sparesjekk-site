...
<div class="ss-step" data-step="4" hidden><p class="ss-kicker">4 av 5</p><h2>Er økonomien blitt vanskelig å håndtere?</h2><p class="ss-help">For eksempel inkasso, betalingsanmerkning eller problemer med å få vanlig refinansiering.</p><div class="ss-options"><button class="ss-choice" data-key="difficulty" data-value="yes">Ja</button><button class="ss-choice" data-key="difficulty" data-value="no">Nei</button></div><div id="ss-security" hidden><h3>Eier du bolig eller kan refinansiering med sikkerhet være aktuelt?</h3><p class="ss-help">Dette spørsmålet vises bare fordi du svarte at økonomien er blitt vanskelig å håndtere.</p><div class="ss-options"><button class="ss-choice" data-key="security" data-value="yes">Ja</button><button class="ss-choice" data-key="security" data-value="no">Nei / usikker</button></div></div></div>
...
document.querySelectorAll('.ss-choice').forEach(function(b){b.onclick=function(){var k=b.dataset.key;state[k]=b.dataset.value;document.querySelectorAll('[data-key="'+k+'"]').forEach(function(x){x.setAttribute('aria-pressed',x===b?'true':'false')});if(k==='difficulty'){var sec=document.getElementById('ss-security');sec.hidden=b.dataset.value!=='yes';if(b.dataset.value!=='yes'){delete state.security;document.querySelectorAll('[data-key="security"]').forEach(function(x){x.setAttribute('aria-pressed','false')})}}}})
...
document.getElementById('ss-next').onclick=function(){
 var need=step===1?['mortgage']:step===2?['mortgageRate']:step===3?['debt']:step===4?(state.difficulty==='yes'?['difficulty','security']:['difficulty']):['card'];
